...
def test_course_id_autogenerated(python100: CatalogCourse) -> None:
    """
    ...
    run = "Fall2026"
    cr = CourseRun.objects.create(catalog_course=python100, run=run)
    cr.refresh_from_db()
    assert isinstance(cr.course_id, CourseLocator)
    assert str(cr.course_id) == f"course-v1:{org_code}+{course_code}+{run}"


def test_course_id_run_match(python100: CatalogCourse) -> None:
    ...
    def create_with(**kwargs):
        ...
        obj = CourseRun.objects.create(catalog_course=python100, run=run, course_id=CourseLocator(**id_args))
        obj.delete()
...
def test_run_unique(python100: CatalogCourse) -> None:
    """
    ...
    CourseRun.objects.create(catalog_course=python100, run=run)
...
def test_run_case_insensitive(python100: CatalogCourse) -> None:
    """
    ...
    """
    CourseRun.objects.create(catalog_course=python100, run="fall2026")
    with pytest.raises(IntegrityError):
        CourseRun.objects.create(catalog_course=python100, run="FALL2026")


def test_run_required(python100: CatalogCourse) -> None:
    """Test that run cannot be blank"""
    course = CourseRun.objects.create(catalog_course=python100, run="fall2026")
    with pytest.raises(IntegrityError):
        # Using .update() will bypass all checks and defaults in save()/clean(), to see if the DB enforces this:
        CourseRun.objects.filter(pk=course.pk).update(run="")


def test_run_exact(org1) -> None:
