...
:ref:`openedx-content-adr-0013` proposes a special type of component that only holds assets (a "File component"), and no XBlock, so that a course's existing files and uploads can be referenced by multiple XBlocks or used independently (e.g. a PDF download).
